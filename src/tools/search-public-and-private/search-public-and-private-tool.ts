...
const searchPublicAndPrivateInputSchema = z.object({
  ...
  channel_types: z
    ...
    .optional()
    .describe(
      "Comma-separated list of channel types. Defaults to 'public_channel,private_channel,mpim,im'",
    ),
  content_types: z
    .string()
    .optional()
...
const searchPublicAndPrivateOutputSchema = z.object({
  query: z.string(),
...
export function createSearchPublicAndPrivateTool(client: WebClient) {
  return tool({
    description: searchPublicAndPrivateDescription,
    inputSchema: searchPublicAndPrivateInputSchema,
    outputSchema: searchPublicAndPrivateOutputSchema,
    execute: async (args) => {
      ...
      type ContextMsg = { user?: string; username?: string; text?: string; ts?: string };
          ...
          .map((m, i) => {
            ...
            if (m.context_before.length > 0) {
              const ctx = m.context_before
                .map((c) => `- From: ${c.user_name} (ID: ${c.user_id}) \n  Message_ts: ${c.ts}\n  ${c.text}`)
                .join("\n");
